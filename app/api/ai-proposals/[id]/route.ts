  ...
  if (action === 'execute') {
    ...
    if (proposal.proposal_type === 'task') {
      const { data, error } = await supabase
        ...
        .insert({
          organization_id: profile.organization_id,
          title: String(p.title || proposal.title),
          description: String(p.description || proposal.summary || ''),
          creator_id: user.id,
      ...
      const { data, error } = await supabase
        ...
        .insert({
          organization_id: profile.organization_id,
          name: String(p.name || proposal.title),
          objective: String(p.objective || proposal.summary || ''),
      ...
      created = { type: 'campaign', ...data };
    } else if (proposal.proposal_type === 'marketplace_job') {
      const { data, error } = await supabase
        .from('marketplace_jobs')
        .insert({
          organization_id: profile.organization_id,
          marketplace_id: p.marketplace_id || null,
          connection_id: p.connection_id || null,
          job_type: String(p.job_type || 'manual_review'),
      ...
      created = { type: 'marketplace_job', ...data };
    } else {
      return NextResponse.json(
        { error: 'This proposal type requires manual execution for now.' },
        { status: 409 }
      );
    }
