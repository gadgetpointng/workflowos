...
export default function PwaInstaller(){
  ...
  useEffect(()=>{
    ...
    const timer=window.setTimeout(()=>{setIos(detectedIos);setVisible(true)},1200);
    const onBeforeInstall=(event:Event)=>{
      event.preventDefault();
      setPrompt(event as InstallPromptEvent);
      setVisible(true);
    };
    const onInstalled=()=>{setVisible(false);setGuideOpen(false);setPrompt(null)};
  ...
  return <>
    <aside aria-label="Install WorkflowOS" className="fixed bottom-5 right-5 z-[100] flex w-[min(390px,calc(100vw-2rem))] items-center gap-3 rounded-xl border border-slate-200 bg-white p-3 shadow-2xl">
      <span className="grid h-11 w-11 shrink-0 place-items-center overflow-hidden rounded-lg bg-[#08111f]"><img src="/icon.svg" alt="" className="h-11 w-11"/></span>
      ...
      <button type="button" onClick={install} className="rounded-md bg-[#2377ff] px-3 py-2 text-xs font-bold text-white">{prompt?'Install':'How'}</button>
    ...
    {guideOpen&&<div className="fixed inset-0 z-[110] grid place-items-center p-5">
      ...
      <section role="dialog" aria-modal="true" aria-labelledby="workflowos-install-title" className="relative w-full max-w-md rounded-2xl bg-white p-6 shadow-2xl">
        <div className="mb-4 grid h-14 w-14 place-items-center overflow-hidden rounded-xl bg-[#08111f]"><img src="/icon.svg" alt="" className="h-14 w-14"/></div>
        ...
        <button type="button" onClick={()=>setGuideOpen(false)} className="mt-5 w-full rounded-lg bg-[#08111f] px-4 py-3 text-sm font-bold text-white">Got it</button>
      </section>
    </div>}
  </>;
}
