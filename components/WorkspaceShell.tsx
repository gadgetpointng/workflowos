...
export default function WorkspaceShell({children,title,subtitle,profile}:{children:ReactNode;title?:string;subtitle?:string;profile?:{full_name?:string|null;role?:string|null;workflowos_identity_source?:string|null;workflowos_access_enabled?:boolean|null;workflowos_permissions?:string[]|null}}){
 ...
 const messageHref=profile?.role==='owner'&&canOpen('/owner-communications')?'/owner-communications':'/inbox';
 ...
 return <div className="min-h-screen bg-[#f7f7f7] text-[#171717]" style={{colorScheme:'light'}}>
  ...
  <Link href={messageHref} aria-label={profile?.role==='owner'?'Open staff messenger':'Open messages'} title={profile?.role==='owner'?'Message staff':'Messages'} className="fixed bottom-[5.9rem] right-4 z-50 flex h-14 w-14 items-center justify-center rounded-full bg-[#102a43] text-2xl text-white shadow-[0_12px_32px_rgba(15,23,42,.28)] ring-1 ring-white/70 transition hover:-translate-y-0.5 hover:bg-[#173f5f] focus:outline-none focus-visible:ring-4 focus-visible:ring-blue-200 lg:bottom-6 lg:right-7">✉<span className="sr-only">{profile?.role==='owner'?'Message staff':'Messages'}</span></Link>
