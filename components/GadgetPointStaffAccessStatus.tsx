...
function enabled(staff: StaffRecord) {
  const metadata = staff.metadata ?? {};
  if (Object.prototype.hasOwnProperty.call(metadata, 'workflowos_access_enabled')) {
    return bool(metadata.workflowos_access_enabled);
  }
  const nested = typeof metadata.workflowos === 'object' && metadata.workflowos
...
export default function GadgetPointStaffAccessStatus({ staff, owner }: { staff: StaffRecord[]; owner: boolean }) {
  const active = staff.filter((item) => String(item.status ?? '').toLowerCase() === 'active');
        ...
        {active.length ? (
          <div className="grid gap-3">
            {active.map((person) => {
              ...
              const identity = userName ? `@${userName}` : person.email || person.external_staff_id || 'staff';
              return (
                <article key={person.external_staff_id || person.id || identity} className="rounded-2xl border border-slate-200 bg-white p-4">
                  <div className="flex flex-wrap items-start justify-between gap-3">
                    <div className="min-w-0">
                      <div className="font-black text-slate-900">{person.full_name || identity}</div>
                      <div className="mt-1 break-all text-xs font-medium text-slate-500">
                        {identity} · {person.role || 'staff'}{person.department ? ` · ${person.department}` : ''}
                      </div>
                    </div>
